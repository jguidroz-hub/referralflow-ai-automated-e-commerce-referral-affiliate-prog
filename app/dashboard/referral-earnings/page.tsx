...
interface ReferralEarningsItem {
  id: string;
  title?: string;
  status?: string;
  createdAt: string;
...
export default function ReferralEarningsPage() {
  const [items, setItems] = useState<ReferralEarningsItem[]>([]);
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/referral-earnings/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Referral Earnings Dashboard</h1>
        <button
      ...
      <p className="text-gray-600 mb-6">Track commissions and payouts</p>
